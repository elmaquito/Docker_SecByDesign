<template>
  <div class="theme-manager">
    <div class="manager-header">
      ...
      <p class="manager-subtitle">Gérez les spécialités et thématiques disponibles pour les notes.</p>
    </div>
    ...
    <div v-else-if="filteredThemes.length === 0" class="empty-state">
      ...
      <p>{{ searchQuery ? 'Aucun thème ne correspond à votre recherche.' : 'Aucun thème créé pour l\'instant.' }}</p>
    ...
    <!-- Themes List -->
    <ul v-else class="themes-list" role="list">
      <li
        v-for="theme in filteredThemes"
        :key="theme.id"
        class="theme-item"
        :style="{ borderLeftColor: theme.meta?.color || 'var(--primary-color)' }"
      >
        <div class="theme-info">
          <div class="theme-name-row">
            <span
              class="theme-badge-dot"
              :style="{ backgroundColor: theme.meta?.color || 'var(--primary-color)' }"
              aria-hidden="true"
            ></span>
            <strong class="theme-name">{{ theme.name }}</strong>
            <span v-if="theme.is_default_for_student_view" class="badge badge-default" title="Visible par défaut pour les étudiants">
              Par défaut
            </span>
          </div>
          <p v-if="theme.meta?.description" class="theme-description">{{ theme.meta.description }}</p>
          <span class="theme-meta-label">Spécialité</span>
        </div>
        <div v-if="canCreate" class="theme-actions">
          <button
            class="btn-icon"
            :title="`Modifier ${theme.name}`"
    ...
    <!-- Create / Edit Modal -->
    <Teleport to="body">
      <div v-if="showModal" class="modal-overlay" role="dialog" aria-modal="true" :aria-labelledby="'modal-title-theme'">
        <div class="modal-content" @click.stop>
          <div class="modal-header">
            <h3 id="modal-title-theme">{{ editingTheme ? 'Modifier le thème' : 'Créer un thème' }}</h3>
            <button class="btn-close" aria-label="Fermer la modale" @click="closeModal">✕</button>
          </div>

          <form class="modal-form" @submit.prevent="saveTheme" novalidate>
            <!-- Name -->
            <div class="form-group">
              <label for="theme-name" class="form-label">Nom <span class="required" aria-hidden="true">*</span></label>
              <input
                id="theme-name"
                v-model="form.name"
                type="text"
              ...
              />
              <span v-if="errors.name" id="theme-name-error" class="error-msg" role="alert">{{ errors.name }}</span>
              <span class="char-count">{{ form.name.length }}/100</span>
            </div>

            <!-- Description -->
            <div class="form-group">
              <label for="theme-desc" class="form-label">Description</label>
              <textarea
                id="theme-desc"
                ...
                aria-describedby="theme-desc-hint"
              ></textarea>
              <span id="theme-desc-hint" class="form-hint">Optionnel · {{ form.description.length }}/500</span>
            </div>
            ...
            <div class="form-group form-group-checkbox">
              <input
                ...
                class="form-checkbox"
              />
              <label for="theme-default" class="form-label-inline">
    ...
    <Teleport to="body">
      <div v-if="themeToDelete" class="modal-overlay" role="alertdialog" aria-modal="true" aria-labelledby="delete-title-theme">
        <div class="modal-content modal-small">
          <div class="modal-header">
            <h3 id="delete-title-theme">Confirmer la suppression</h3>
          </div>
          <p class="delete-warning">
            Êtes-vous sûr de vouloir supprimer le thème <strong>« {{ themeToDelete.name }} »</strong> ?
            Cette action est irréversible.
          </p>
          <div v-if="deleteError" class="alert alert-error" role="alert">{{ deleteError }}</div>
          <div class="modal-footer">
            <button class="btn-secondary" @click="themeToDelete = null">Annuler</button>
...
const editingTheme = ref<Tag | null>(null)
const themeToDelete = ref<Tag | null>(null)
...
const filteredThemes = computed(() => {
  ...
  return tagStore.specialites.filter(t =>
    !query || t.name.toLowerCase().includes(query) || (t.meta?.description ?? '').toLowerCase().includes(query)
  )
})
...
function openEditModal(theme: Tag) {
  editingTheme.value = theme
  form.value = {
    name: theme.name,
    description: theme.meta?.description ?? '',
    color: theme.meta?.color ?? COLOR_OPTIONS[0].value,
    isDefault: theme.is_default_for_student_view,
  }
  errors.value = {}
  saveError.value = ''
  showModal.value = true
...
async function saveTheme() {
  ...
  try {
    ...
    if (editingTheme.value) {
      await tagStore.updateTag(editingTheme.value.id, payload)
    } else {
...
function confirmDelete(theme: Tag) {
...
async function executeDelete() {
  ...
  deleteError.value = ''
  try {
    await tagStore.deleteTag(themeToDelete.value.id)
    themeToDelete.value = null
  } catch (e: any) {
    deleteError.value = e.message || 'Impossible de supprimer ce thème.'
  } finally {
...
@media (max-width: 480px) {
  .theme-manager { padding: 1rem 0.5rem; }
  .toolbar { flex-direction: column; align-items: stretch; }
  .btn-primary { justify-content: center; }
  .theme-item { flex-direction: column; gap: 0.5rem; }
